...
// Renders the policy's `elements` array. Each element has a `variants[]`
// dropdown sourced from the KB, plus Custom override / Reset to canonical.
...
export function buildPreviewDisclosuresCustomJS(block: BlockBase, ctx: BuildContext): string {
  ...
  // Per-element visible variants (filtered by context.product) for runtime lookup.
